<script>
  ...
  import InputField from "./IndexField.svelte";

  export let err = true;
  export let msg = "";
  const dispatch = createEventDispatcher();
  const defobj = { indexfield: "", type: "", error: false };
  let values = [{ indexfield: "", type: "", error: false }];
  ...
  let input = {
    errorCheck: false,
    Addfield: () => {
      values.push({ indexfield: "", type: "", error: false });
      console.log(values);
      values = values;
      input.Checkedtog();
    },
    Removefield: (event) => {
      let index = event.detail?.index ?? false;
      console.log(index);
      if (index === false) {
        return;
      }
    ...
    Checkedtog: () => {
      input.errorCheck = !input.errorCheck;
    },
    validateALL: () => {
      if (values.length <= 0) {
        return false;
  ...
  <div
    ...
    <div class="fixed inset-0 z-10 overflow-y-auto">
      <div
        ...
        <div
          class="relative transform overflow-hidden rounded-lg bg-mongodark-100 text-left transition-all sm:my-8 sm:w-full sm:max-w-2xl">
          <div class="p-9">
            <div
              class="card-title text-mongo-card-textcolor font-bold text-2xl mb-7">
              Create Indexes
            </div>
            <div class="flex gap-2 flex-col">
              {#each values as value, index}
                <InputField
                  bind:indexFields={values.length}
                  bind:value
                  bind:error={input.errorCheck}
                  {index}
                  on:Removefield={input.Removefield}
                  on:Addfield={input.Addfield} />
              {/each}
            </div>
            <slot />
          </div>
          {#if msg}
            <div class="px-9 ">
              <div
                class="text-red-600 border border-solid border-red-700 font-xs p-5">
                {msg}
              </div>
            </div>
          {/if}
          ...
          <div
            ...
            <button
              on:click={submit}
              class:bg-mongo-green-btn={!err}
              class:border-mongo-card-btnborder={err}
              class:border-transparent={!err}
              disabled={err}
              type="button"
              class="inline-flex w-full justify-center  rounded-md border   px-4 py-2 text-base font-medium text-mongo-white shadow-sm hover:bg-mongo-green-hover hover:text-mongo-white transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm"
              >Create Index</button>
            <button
              on:click={cancel}
              type="button"
